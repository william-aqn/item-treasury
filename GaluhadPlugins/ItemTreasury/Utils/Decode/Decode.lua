...
-- Retrieve what info we can
	...
	if isLI then
		...
		if nStats ~= 0 and type(nStats) == 'number' then
			local stats = {};
			for i = 1, nStats do
				local ID = ins:GetLongLE();
	...
	for i = 1, nSubs do
		...
		elseif header == 0x100012C5 then
			...
			for i = 1, nExtras do
				...
				elseif header == 0x10005F0E then -- socketted item
					local essences = {};
					local nSockets = ins:GetLongLE();
					for i=1, nSockets do
						if ins:GetLongLE() == 0x10005F3D then	-- Essence Header
							local subEssence = {};
							ins:Consume(1);
							local nEssenceDetails = ins:Get();	-- No. details in array
							for j=1,nEssenceDetails do
								local headerEs1 = ins:GetLongLE();
								local headerEs2 = ins:GetLongLE();
								if headerEs1 == 0x10005F05 then -- Essence Generic Item ID
									subEssence.EssenceID = ins:GetLongLE();
								elseif headerEs1 == 0x10005F3E then -- Essence Item Level
									subEssence.EssenceLevel = ins:GetLongLE();
								else
									ins:Consume(4);
								end
							end
							table.insert(essences,subEssence);
						end
					end
					result.Essences = essences;
				end
			end
