...
function AutoSizeToolTip(SCROLLABLECONTROL)
	...
	local function SwitchToHeight()
		SCROLLABLECONTROL:SetHorizontalScrollBar();
		SCROLLABLECONTROL:SetWidth(maxWidth);
		SCROLLABLECONTROL:SetMultiline(true);
		SCROLLABLECONTROL:SetText(SCROLLABLECONTROL:GetText());
		...
		if vScroll:IsVisible() then
			...
			while vScroll:IsVisible() == true and iCount < 1000 do
	...
	if hScroll:IsVisible() == true then
		...
		while hScroll:IsVisible() == true and iCount < 1000 do
			...
			iCount = iCount + 1;
			if SCROLLABLECONTROL:GetWidth() > maxWidth then
				SwitchToHeight();
				break;
			end
		end
	...
	SCROLLABLECONTROL:SetHorizontalScrollBar();
	SCROLLABLECONTROL:SetVerticalScrollBar();
